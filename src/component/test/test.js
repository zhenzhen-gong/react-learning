...
class App extends Component {
   ...
   handleSubmit = (e)=>{
     // 阻止form表单的提交事件
      e.preventDefault()
      console.log(this.state.person, this.state.sex, this.state.cityName, this.state.info)
   }
   render() {
     return (
          ...
          城市：<select value={this.state.cityName} onChange={this.handleCity}>
            ...
            爱好：{
              this.state.hobby.map((item, key) => {
                return (
                  <span  key={key}>
                    <input type="checkbox" key={key} checked={item.checked} onChange={this.handleHobby.bind(this,key)} />{item.title}
                  </span>
                )
              })
            }
            ...
            <div>
              留言板：<textarea value={this.state.info} onChange={this.handleInfo}/>
            </div>
